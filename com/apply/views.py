...
def team(request):  # 填写队伍信息
    try:
        ...
        new_team = Team.objects.create(T_name=T_name, T_intro=T_intro, T_slogan=T_slogan, T_leader=T_leader,
                                       T_phone=T_phone,
...
def searchMember(request):
    ...
    if type == 0:
        ...
        obj = teachers.objects.filter(t_phone=phone, t_name=name)
    return obj, phone, team_id, type


def addMatchMember(request):  # 添加比赛队友
    try:
...
def deleteMessage(request):
    try:
        ...
        if types == 0:
            id = students.objects.get(s_phone=phone).id  # 先查询该学生的id
        else:
            id = teachers.objects.get(t_phone=phone).id
        Member.objects.get(member_id=id, team_id=team_id, type=types).delete()  # 在member表里进行删除
        return HttpResponse('<script>alert("删除成功！")</script>')
    except:
        return HttpResponse('<script>alert("用户不存在或已删除~")</script>')
